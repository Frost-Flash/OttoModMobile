...
import ottoMod.cards.ottogreen.Strike_Otto;
import ottoMod.enums.CardColorEnum;
import ottoMod.relics.Rusted_Wheelchair;

import java.util.ArrayList;

import static ottoMod.OttoMod.getResourcePath;
import static ottoMod.OttoMod.makeId;

// 继承CustomPlayer类
public class Mycharacter extends CustomPlayer {
    // 火堆的人物立绘（行动前）
    private static final String MY_CHARACTER_SHOULDER_1 = getResourcePath("char/shoulder3.png");
    // 火堆的人物立绘（行动后）
    private static final String MY_CHARACTER_SHOULDER_2 = getResourcePath("char/shoulder4.png");
    // 人物死亡图像
    private static final String CORPSE_IMAGE = getResourcePath("char/corpse1.png");
    // 战斗界面左下角能量图标的每个图层
    ...
    protected void initializeClass(String imgUrl, String shoulder2ImgUrl, String shouldImgUrl, String corpseImgUrl, CharSelectInfo info, float hb_x, float hb_y, float hb_w, float hb_h, EnergyManager energy) {
        if (imgUrl != null) {
            this.img = AssetLoader.getTexture(OttoMod.MOD_ID,imgUrl);
        }
        ...
        this.shoulderImg = AssetLoader.getTexture(OttoMod.MOD_ID,shouldImgUrl);
        this.shoulder2Img = AssetLoader.getTexture(OttoMod.MOD_ID,shoulder2ImgUrl);
        this.corpseImg = AssetLoader.getTexture(OttoMod.MOD_ID,corpseImgUrl);
    ...
    public Mycharacter(String name) {
        super(OttoMod.MOD_ID,name, CardColorEnum.otto,null,getResourcePath("orb/vfx.png"), null, null, null);
        ...
        this.initializeClass(
                getResourcePath("char/character1.png"), // 人物图片
    ...
    public ArrayList<String> getStartingDeck() {
        ...
        for(int x = 1; x < 5; ++x) {
            retVal.add("Strike_Otto");
        }

        for(int x = 1; x < 5; ++x) {
            retVal.add("Defend_Otto");
        }

        retVal.add("Wheelchair_dashing");
        retVal.add("Dumpling");
        return retVal;
    }
    // 初始遗物的ID，可以先写个原版遗物凑数
    public ArrayList<String> getStartingRelics() {
        ArrayList<String> retVal = new ArrayList<>();
        retVal.add(Rusted_Wheelchair.ID);
        return retVal;
    }
    ...
    public AbstractCard.CardColor getCardColor() {
        return CardColorEnum.ottogreen;
    }

    // 翻牌事件出现的你的职业牌（一般设为打击）
    @Override
    public AbstractCard getStartCardForEvent() {
        return new Strike_Otto();
    }

    // 卡牌轨迹颜色
    @Override
    public Color getCardTrailColor() {
        return OttoMod.YELLOW_COLOR;
    }
